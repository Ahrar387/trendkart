...
interface Props { searchParams: Promise<{ q?: string; sortBy?: string; page?: string }> }
...
export default async function SearchPage({ searchParams }: Props) {
  ...
  const result = query
    ? await getProducts({ search: query, sortBy, page, pageSize: 12 })
    ...
    <div className="container-site py-8 space-y-6">
      <div>
        <h1 className="section-title">
          {query ? <>Results for <span className="text-brand-500">&ldquo;{query}&rdquo;</span></> : "Search Products"}
        </h1>
        {query && <p className="text-sm text-gray-500 mt-1">{result.total.toLocaleString("en-IN")} products found</p>}
      </div>

      {!query ? (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          <Search className="h-16 w-16 text-gray-200 mb-4" />
          <h2 className="text-xl font-semibold text-gray-700 mb-2">Search for anything</h2>
          <p className="text-gray-400">Try &ldquo;iPhone&rdquo;, &ldquo;Sony headphones&rdquo;, &ldquo;Nike shoes&rdquo;…</p>
        </div>
        ...
        <>
          <Suspense><FilterBar currentSort={sortBy} showCategoryFilter={false} /></Suspense>
